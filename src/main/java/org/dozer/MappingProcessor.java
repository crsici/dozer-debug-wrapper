...
public class MappingProcessor implements Mapper {
  ...
  /* Mapper Interface Implementation */
    ...
    public <T> T map(final Object srcObj, final Class<T> destClass, final String mapId) {
        System.out.println(String.format("Mapping %s -> %s",srcObj.getClass().getCanonicalName(),destClass.getCanonicalName()));
...
//            System.out.println( String.format("Mapping field %s(%s) -> %s(%s)",fieldMapping.getSrcFieldName(), fieldMapping.getDestFieldName()));
    ...
    private void mapFromFieldMap(Object srcObj, Object destObj, Object srcFieldValue, FieldMap fieldMapping) {
        ...
        if (!MappingUtils.isBlankOrNull(fieldMapping.getCustomConverterId())) {
            if (customConverterObjectsWithId != null && customConverterObjectsWithId.containsKey(fieldMapping.getCustomConverterId())) {
                ...
                System.out.println( String.format("Mapping field %s(%s) -> %s(%s)",fieldMapping.getSrcFieldName(),srcFieldValue.getClass().getCanonicalName(), fieldMapping.getDestFieldName(),destFieldType));
    ...
    private Object mapCustomObject(FieldMap fieldMap, Object destObj, Class<?> destFieldType, Object srcFieldValue) {
        ...
        System.out.println(String.format("Map nest object %s(%s) -> %s(%s)",srcFieldValue,srcFieldValue.getClass(), fieldMap.getDestFieldName(),  destFieldType.getCanonicalName()));
